...
if __name__ == '__main__':
    ...
    img_dir = "./data/Room.jpg"

    grid_search = False
    if not grid_search:
        L = 6
        model = NN.MLP(4 * L, 512)
        # model = resnet.ResNet1D()
        # model = Siren.Siren(in_features=2, out_features=1, hidden_features=256, hidden_layers=3, outermost_linear=True, first_omega_0=50., hidden_omega_0=50.)
        train_tools.train_and_evaluate(img_dir, model, N=4, batch_size=8, num_epochs=9999, earlyStoppingEpoches=40, device=0, date=date_org, encoding="Sine", sine_L=L)
    else:
        ...
        for L in Ls:
            for Size in Sizes:
                ...
                mse, psnr, ssim = train_tools.train_and_evaluate(img_dir, model, N=4, batch_size=8, num_epochs=9999, earlyStoppingEpoches=40, device=0, date=date, encoding="Sine", sine_L=L)
                GRID_MSE[Size][L] = mse
